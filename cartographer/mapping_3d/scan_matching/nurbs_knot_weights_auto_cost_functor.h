/*
 * nurbs_knot_weights_auto_cost_functor.h
 *
 *  Created on: Aug 25, 2017
 *      Author: schnattinger
 */

#ifndef CARTOGRAPHER_MAPPING_3D_SCAN_MATCHING_NURBS_KNOT_WEIGHTS_AUTO_COST_FUNCTOR_H_
#define CARTOGRAPHER_MAPPING_3D_SCAN_MATCHING_NURBS_KNOT_WEIGHTS_AUTO_COST_FUNCTOR_H_
...
class NurbsKnotWeightsAutoCostFunctor {
 public:
  ...
  NurbsKnotWeightsAutoCostFunctor(const NurbsKnotWeightsAutoCostFunctor&) = delete;
  NurbsKnotWeightsAutoCostFunctor& operator=(const NurbsKnotWeightsAutoCostFunctor&) = delete;
  ...
  bool operator()(const T* const point_1_trans, const T* const point_1_rotation,
    ...
    boost::multi_array<T, 1>& weights_estimate = nurbs.getWeights().getWeights();
    for (int i = 0; i < weights_estimate.size(); i++) {
      weights_estimate[i] = weights_vec[i];
      }
...
#endif /* CARTOGRAPHER_MAPPING_3D_SCAN_MATCHING_NURBS_KNOT_WEIGHTS_AUTO_COST_FUNCTOR_H_ */
