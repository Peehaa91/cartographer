...
{
  const bool initial_plane_data = (ground_plane_tracker_ == nullptr);
  if (initial_plane_data){
    ground_plane_tracker_ = common::make_unique<mapping_3d::GroundPlaneTracker>();
  }
  Predict(time);
  ground_plane_tracker_->AddGroundPlaneObservation(coefficients);
  pose_estimate_ = transform::Rigid3d::Rotation(ground_plane_tracker_->orientation());
}
...
void LocalTrajectoryBuilder::Predict(const common::Time time) {
  ...
  if (!options_.use_plane()){
    ...
    const Eigen::Quaterniond last_orientation = ground_plane_tracker_->orientation();
    if (time_ > common::Time::min()) {
      const double delta_t = common::ToSeconds(time - time_);
      // Constant velocity model.
      const Eigen::Vector3d translation =
